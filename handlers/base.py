...
async def send_welcome(message: types.Message):
    ...
    if str(user.id) not in all_ids:
        await message.answer(
            f"""Привет, {user.first_name}! 👋 Мы рады вас видеть в Greenmania 🌿
Вы поймали скидку 500₽ на массаж — уже знаете, какой хотите?

У нас большой выбор! Пока вы думаете, я передал ваш контакт администратору. 
Она скоро свяжется с вами и подберёт удобное для вас время 😊""",

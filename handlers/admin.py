...
async def handle_reply_button(callback_query: types.CallbackQuery):
    ...
    if admin_id not in ADMIN_IDS:
        ...
        return
    # Extract target user ID and enter reply mode
...
async def handle_admin_reply(msg: types.Message):
    ...
    try:
        ...
        logger.info(f"Админ {msg.from_user.full_name} ответил пользователю {user_id}: {reply_text}")
    except Exception as e:
        await msg.reply(f"❌ Не удалось отправить сообщение пользователю: {e}")
        return

    # 2) Record reply and timestamp in Google Sheet
    ...
    # 3) Broadcast this reply to other admins so everyone sees it
